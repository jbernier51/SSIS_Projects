...
CREATE   VIEW [Harsco].[vDimDateARReceipts]
AS
...
END AS [Day-Month-Year]
  ...
  FROM [Harsco].[DimDate]
  WHERE datekey >= (SELECT MIN(ReceivedDateKey) FROM harsco.FactARReceipts )
  AND datekey <= (SELECT MAX(ReceivedDateKey) FROM harsco.FactARReceipts )
