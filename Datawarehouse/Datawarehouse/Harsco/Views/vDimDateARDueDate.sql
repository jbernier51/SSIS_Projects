...
CREATE   VIEW [Harsco].[vDimDateARDueDate]
AS
...
END AS [Day-Month-Year]
  ...
  FROM [Harsco].[DimDate]
  WHERE datekey >= (SELECT MIN(InvoiceDueDateKey) FROM harsco.FactARInvoices )
  AND datekey <= (SELECT MAX(InvoiceDueDateKey) FROM harsco.FactARInvoices )
