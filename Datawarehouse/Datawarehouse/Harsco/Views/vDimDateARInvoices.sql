...
CREATE   VIEW [Harsco].[vDimDateARInvoices]
AS
...
END AS [Day-Month-Year]
  ...
  FROM [Harsco].[DimDate]
  WHERE datekey >= (SELECT MIN(InvoiceDateKey) FROM harsco.FactARInvoices )
  AND datekey <= (SELECT MAX(InvoiceDateKey) FROM harsco.FactARInvoices )
