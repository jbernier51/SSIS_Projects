...
CREATE   VIEW [Harsco].[vDimDateCustomerNotes]
AS
...
END AS [Day-Month-Year]
  ...
  FROM [Harsco].[DimDate]
  WHERE datekey >= (SELECT MIN(NoteDateKey) FROM harsco.FactCustomerNotes )
  AND datekey <= (SELECT MAX(NoteDateKey) FROM harsco.FactCustomerNotes )
